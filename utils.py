...
def Fill_Figure(width, height, size, xi, yi, r, g, b):
	r, g, b = 255 * r, 255 * g, 255 * b
	stack = []
	stack.append((xi, yi))
	while len(stack) > 0:
...
def Fill_Figure_2(width, height, size, xi, yi, r, g, b):
	r, g, b = 255 * r, 255 * g, 255 * b
	init = color_pixel(width, height, xi, yi, size)
	stack = []
	stack.append((xi, yi))
	while len(stack) > 0:
		x, y = stack.pop()

		if color_pixel(width, height, x, y, size) == init:
			set_pixel(x, y, r, g, b, size)

		# examine surrounding pixels to see if they should be placed onto stack
		if color_pixel(width, height, x + 1, y, size) == init:
			stack.append((x + 1, y))

		if color_pixel(width, height, x, y + 1, size) == init:
			stack.append((x, y + 1))
...
def Escalation(vertices, tx, ty, sx, sy):
	T = [
		[sx, 0 , -1*tx*sx+tx], 
		[0 , sy, -1*ty*sy+ty], 
		[0 , 0 , 1]
...
def ReflectionY(vertices):
	...
	for item in vertices:
		...
		result.append(point)
	return result
def Reflection(vertices, angle):
	vertices_2 = Rotation(vertices, -1*angle)
	vertices_2 = ReflectionX(vertices_2)
	result = Rotation(vertices_2, angle)
	return result

def DeformationX(vertices, defx):
...
def DeformationY(vertices, defy):
...
def Deformation(vertices, defy, angle):
	
	
	vertices_2 = Rotation(vertices, -1*angle)
	vertices_2 = DeformationX(vertices_2,defy)
	result = Rotation(vertices_2, angle)
	return result
